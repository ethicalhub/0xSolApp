...
const rpcCall = async (method: string, params: any[]) => {
  console.log(`[rpcCall] method=${method}`, params);
  ...
  console.log(`[rpcCall] raw response for ${method}:`, JSON.stringify(data));

  if (data.error) {
    console.error(`[rpcCall] RPC error for ${method}:`, data.error);
...
const getBalance = async (publicKey: string) => {
  ...
  console.log(
    `[getBalance] lamports=${result.value}, SOL=${result.value / 1_000_000_000}`,
  );
  return result.value / 1_000_000_000;
};

const getTokens = async (publicKey: string) => {
  ...
  console.log(
    `[getTokens] raw accounts count=${result.value?.length ?? 0}`,
    result.value,
  );
  ...
  console.log(`[getTokens] filtered tokens count=${filtered.length}`, filtered);
  return filtered;
};

const getTxns = async (publicKey: string) => {
  const sigs = await rpcCall("getSignaturesForAddress", [
    publicKey,
    { limit: 10 },
  ]);
  console.log(`[getTxns] raw sigs count=${sigs?.length ?? 0}`, sigs);
...
export default function Index() {
  ...
  const search = async () => {
    ...
    try {
      ...
      console.log(
        `[search] results — balance=${bal}, tokens=${tok.length}, txns=${tx.length}`,
      );
      setBalance(bal);
      setTokens(tok);
      setTxns(tx);
      ...
      <ScrollView style={s.scroll}>
        ...
        {balance !== null && (
          <View style={s.card}>
            ...
            <Text style={s.addr}>{short(publicKey.trim(), 6)}</Text>
          </View>
        )}
